...
function App() {
  ...
  const startTimer = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (timeDifference <= 0) {
        ...
        // Call function to announce winner or perform any other action
        announceWinner();
      }
    }, 1000);
  };
  ...
  const announceWinner = () => {
    let maxVotes = Math.max(Count1, Count2, Count3);
    let winner = "";
    if (maxVotes === Count1) {
      winner = "Cristiano Ronaldo";
    } else if (maxVotes === Count2) {
      winner = "Lionel Messi";
    } else {
      winner = "Kylian Mbappe";
    }
    setWinner(winner);
    alert(`Ohh you are late, voting has ended. The winner is ${winner}`);
  };
      ...
      <div className="dataContainer">
        ...
        <div className="timer">
          {timerRunning ? (
            <div>
              Timer:{" "}
              {endTime && new Date(remainingTime).toISOString().substr(11, 8)}
            </div>
          ) : (
